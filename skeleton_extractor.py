...
# STEP (2/3): Script to process images in images folder

# Video input
inputVideo = cv.VideoCapture("cam7.avi")
number = -1
# Webcam Input
#inputVideo = cv.VideoCapture(0)
readSuccussful, frame = inputVideo.read()
...
# plt.ion()
while readSuccussful:
    try:
	readSuccussful, oriImg = inputVideo.read()
	...
	number+=1
        cv.imwrite('results/' + "skeleton" + str(number) + ".jpg", canvas[:, :, [0, 1, 2]])
        with open('results/' + "skeleton" + str(number) + '.txt', 'w') as f:
            for z in range(len(subset)):
                for q in range(18):
                    index = subset[z][q]
                    if -1 == index.astype(int):
                        f.write(str(-1) + ' ' + str(-1) + '\n')
                        continue
                    Y = candidate[0, 0]
                    X = candidate[index.astype(int), 1]
